...
// 2. Mybatis 프레임워크의 환경설정파일 작성(mybatis-config.xml - settings 엘리먼트)
// => [src/main/webapp] 폴더에 작성해야만 스프링 컨테이너(WebApplicationContext 객체)가
// Mybatis 프레임워크의 환경설정파일을 읽어서 객체 SqlSessionFactory 클래스를 Spring Bean으로 등록 처리
// => [src/main/java] 또는 [src/main/resources] 폴더에 Mybatis 프레임워크의 환경설정파일 작성
...
// bean 엘리먼트로 해당 엘리먼트로 클래스를 Spring Bean으로 등록 - root-context.xml, 또는 servlet-context.xml
...
// DAO 클래스는 Service 클래스의 객체로 제공받아 사용되도록 반드시 Spring Bena으로 등록
...
public class StudentDAOImpl implements StudentDAO {
	// Mybatis 프레임워크를 사용하여 DAO 클래스를 작성할 경우 메퍼에 등록된 SQL 명령을 제공받아
