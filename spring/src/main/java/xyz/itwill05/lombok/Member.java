...
// 2. 이클립스를 종료하고 콘솔(명령 프롬프트 - cmd)를 관리자 권한으로 실행한 후 Lombok 라이브러리가
// 저장된 로컬저장소(Local Repository - 사용자 폴더의 .m2 폴더)의 라이브러리 폴더로 이동
// 3. 콘솔에서 Jar 프로그램 실행하여 Lombok 라이브러리를 이클립스에서 사용할 수 있도록 설정
...
// => 이클립스가 실행되지 않는 경우 이클립스 설치 폴더로 이동하여 eeclipse.ini 파일 변경
// => eclipse.ini 파일에서 javaagent 속성의 lombok 경로를 절대경로에서 상대경로로 변경 -javaagent:lombok.jar
